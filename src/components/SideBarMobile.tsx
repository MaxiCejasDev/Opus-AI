...
export default function SideBarMobile({handleUserModal,userModal,handleToggleSideBar,toggleSideBar}:Props){
    ...
    return(
        ...
        <button onClick={handleToggleSideBar} className="fixed md:hidden w-[30px] h-[30px] flex items-start justify-start top-[16px] left-[16px] z-20">
            <div className="h-[6px] w-full flex flex-col gap-y-[2px] relative">
                <span className={`h-[2px] w-[30px] absolute top-0 left-0 duration-300 bg-white ${toggleSideBar?'w-[15px]':''}`}></span>
